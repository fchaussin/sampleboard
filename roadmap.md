...
| 26 | **English by default + language switch**: `en.json` complete (default & fallback — decision §16), `fr.json` kept complete, selector already in Settings (registry-driven). Factory-manifest labels switched to English (visible pad data), fastlane screenshots regenerated under **en-US** (+ fastlane `icon.png` in both locales, new 3×3 icon), README/docs point to the EN captures, e2e adapted (EN chips, dot decimals). **Shipped on 2026-07-05 (on request).** | 2026-07-05 | — | Shipped |
| 27 | **Buffer preloader/pacer**: the app mounts WITHOUT waiting for audio decoding — buffers load by priority (active-page pads → other pads → library), 3 concurrent decodes; a pad whose buffer is pending shows a **« loading » state** (discreet pulse) instead of being a silent no-op; a failed load settles too (silent no-op per §12). Factory seeding now assigns each board slot **as soon as its sample lands** (progressive board, no more end-of-seed pop-in; multi-pad slots preserved). `app/preload.ts` + `store.pendingBufferIds` + PadStatus 'loading'. **Shipped on 2026-07-05 (on request)** — 4 unit tests (priority, bounded concurrency, settle-on-failure) + suites green. | 2026-07-05 | — | Shipped |
| 28 | **Second factory board — 3×3 drum kit on page 2**: the manifest generalizes to **`boards` per page rank** (optional imposed grid; seeding resizes the page via `setPageGrid` BEFORE mapping slots; duplicates = multi-pad allowed; legacy `board` key rejected at build). 9 CC0 drum one-shots (Kick, Snare, Clap, Closed/Open hi-hat, Shaker, Tom, Crash, Ride), new default tag **Drums** (token `drums`, i18n EN/FR). Rebank tooling now **incremental**: deposited files are reused verbatim (never re-picked) unless `--only`. **Shipped on 2026-07-05 (on request)** — build-validation + seed tests updated (+progressive slot test), e2e page-2 assertions; 322 unit + 19 e2e green. | 2026-07-05 | — | Shipped |
| 29 | **Edit mode = one icon + violet everywhere**: the segmented Play/Edit toggle becomes a single compact **Edit icon** (neutral at rest — it is the entry button, not a disabled control; solid violet when armed; re-tap returns to Play; `.mode-toggle` selector kept for e2e). The Edit violet now covers every Edit surface: pads (existing MIDI-map), **pool** (violet wash + border — Edit-only tool), and a **subtle inner ring on the library** when Edit is armed (it serves both modes). **Shipped on 2026-07-05 (on request, design discussed)** — visual captures validated; 322 unit + 19 e2e green. | 2026-07-05 | — | Shipped |
| 30 | **Global Stop lives PERMANENTLY next to the master visualizer** (topbar, right of the waves — the visualizer shows what sounds, Stop cuts it, same spot in every view): discreet at rest, red-armed while anything plays; removed from the bottombar (single home, more room for page tabs). e2e adapted (`.topbar .stop`). **Shipped on 2026-07-05 (on request).** | 2026-07-05 | — | Shipped |
...
| 35 | **« Pads » (board) button in the bottombar**: an explicit view switch beside the Library button — the two toggle `<main>` between the board (grid) and the library, the active one shown in accent. « Pads » reuses `closeLibrary` (returns to the board, symmetric with the ✕ / Android back). New `grid` icon. **Shipped on 2026-07-05 (on request).** e2e extended (active-view + return to board). | 2026-07-05 | — | Shipped |
