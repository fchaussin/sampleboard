...
| 23 | **Navigation pilotée par l'URL** : la vue affichée devient une projection de l'URL (invariant : jamais une variable indépendante — remplace le booléen `libraryOpen`). Table `vue → composant`, résolution avec défaut (board), rendu dynamique, sync bidirectionnelle (init/`hashchange`/retour-avance ↔ écriture d'URL uniquement). Paramètres arbitrés et figés en spec §16 : **fragment `#` + historique délibéré** (push navigation / `replace` redirections — geste retour Android cohérent) ; **cardinalité vue + paramètres** (ex. `#/library?tag=…`). | 2026-07-04 | — | À trier |
